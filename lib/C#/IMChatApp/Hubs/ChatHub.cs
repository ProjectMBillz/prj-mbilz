...
using Newtonsoft;
using QuickClaims.Core.Data;
using QuickClaims.Services;
...
namespace IMChatApp.Hubs
{
    ...
    public class ChatHub : Hub
    {
        ...
        {
            //var user = new UserSystem().RetrieveUserByUsername(name);
            //user.ChatID = Context.ConnectionId;
        ...
        public void UpdateStatus(string status)
        {
            ...
            //loggedInUsers.FirstOrDefault(x => x.ChatID == userId).status =status;
            //var fromUser = loggedInUsers.FirstOrDefault(x => x.ConnectionId == fromUserId);                          
